...
def emojified(guess_word: str, secret_word: str) -> str:
    ...
    while idx < len(guess_word):
        if guess_word[idx] == secret_word[idx]:
            emojis += green_box  # adds a box to the string of emojis
        else:
            if contains_char(secret_word, guess_word[idx]):
                emojis += yellow_box  # adds a box to the string of emojis
            else:
                emojis += white_box  # adds a box to the string of emojis
        idx += 1
    return emojis  # the completed string of emojis is returned 
...
def input_guess(expected_length: int) -> str:
    ...
    guess_word = input(f"Enter a {expected_length} letter word: ")  # prompts the player to enter a word
    while len(guess_word) != expected_length:  # while the length of the guess word isnt equal to length of the secret word
...
def main() -> None:
    ...
    the_word: str = ("codes")  # THE SECRET WORD
    length: int = len(the_word)  # creates a variable adapting to the length of the word

    while tries < max_tries: 
        print(f"==Turn {tries + 1}/{max_tries}==")
        guess_word: str = input_guess(length)  # creates a variable to the word guessed by the player, locally to change with each loop and continue to prompt the player
